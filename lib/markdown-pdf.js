...
module.exports = {
  ...
  config: {
    "rendering":{
      ...
      "description": "Render the markdown to html with Marked or Markdown-Preview.",
      "type": "string",
      "default": "Markdown-Preview",
      "enum": ["Marked", "Markdown-Preview"]
    },
    "format": {
      "title": "Page Format",
      "type": "string",
...
function getStyles(){
  var user_styles = fs.readFileSync(atom.styles.getUserStyleSheetPath(), 'utf8');
  ...
  var styles = {
    ...
    uiStyles: atom.themes.getActiveThemes()[1].stylesheets[0][1],
...
function makeHtml(outputPath){
  pandoc = childProcess.spawn('pandoc', [atom.workspace.activePaneItem.getPath(),
      '-f', 'markdown', '-t', 'html5', '-s', '-S', '--self-contained', '--ascii',
      '--toc', '-o', outputPath],
...
function makePdf(outputPath){
  getHtml(function(html){
    ...
    console.log(html);
    pdf.create(html, getConfig(), function(err, buffer) {
      if (err) {
...
function make(outputPath){
  var ext = getExtension(outputPath);
  if(ext == "html"){
    makeHtml(outputPath);
  }
  else if(ext != "html"){
